...
{
  // Đặt giá trị "nice" của luồng hiện tại thành giá trị được chỉ định.
  thread_current()->nice = nice;

  // Khi "nice" được thay đổi, ưu tiên của luồng có thể thay đổi.
  // Gọi hàm thread_update_priority_mlfqs để cập nhật ưu tiên của luồng hiện tại.
  thread_update_priority_mlfqs(thread_current());

  // Nếu có luồng có ưu tiên cao hơn luồng hiện tại, gọi hàm try_thread_yield để nhường CPU.
  try_thread_yield();
}
...
{
  // Gọi hàm macro FP_MULT_MIX để nhân giá trị của load_avg với hằng số 100.
  // Sau đó, gọi hàm macro FP_ROUND để làm tròn kết quả.
...
{
  // Lưu trạng thái ngắt hiện tại và tắt ngắt.
  enum intr_level old_level = intr_disable();
  
  // Kiểm tra xem danh sách các luồng sẵn sàng có trống hay không và
  // ưu tiên của luồng ở cuối danh sách có lớn hơn ưu tiên của luồng hiện tại hay không.
  bool result = !list_empty(&ready_list) &&
                list_entry(list_back(&ready_list), struct thread, elem)->priority >
	            thread_get_priority();

  // Khôi phục trạng thái ngắt về giá trị ban đầu.
  intr_set_level(old_level);

  // Nếu kết quả là đúng, gọi hàm thread_yield() để nhường CPU cho luồng khác.
  if (result)
    thread_yield();
}
...
{
  // Tạm thời tắt ngắt để đảm bảo tính toàn vẹn của dữ liệu.
  enum intr_level old_level = intr_disable();
