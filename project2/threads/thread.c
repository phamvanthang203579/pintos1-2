...
tid_t thread_create(const char *name, int priority, thread_func *function, void *aux) {
  ...
  /* Khởi tạo cho tiến trình con của tiến trình. */
  ...
  /* Khởi tạo trạng thái exit của tiến trình con với giá trị MAX để sửa lỗi. */
  t->thread_child->store_exit = UINT32_MAX;
  t->thread_child->isrun = false;

  /* Khung ngăn xếp cho kernel_thread(). */
  kf = alloc_frame(t, sizeof *kf);
  kf->eip = NULL;
  kf->function = function;
  kf->aux = aux;

  /* Khung ngăn xếp cho switch_entry(). */
  ef = alloc_frame(t, sizeof *ef);
  ef->eip = (void (*)(void))kernel_thread;

  /* Khung ngăn xếp cho switch_threads(). */
  sf = alloc_frame(t, sizeof *sf);
  sf->eip = switch_entry;
  sf->ebp = 0;

  /* Thêm vào hàng đợi chạy. */
  thread_unblock(t);
...
/* Loại bỏ luồng khỏi danh sách tất cả các luồng, đặt trạng thái của chúng ta là dying,
   và lên lịch cho một quá trình khác. Quá trình đó sẽ phá hủy chúng ta
   khi nó gọi thread_schedule_tail(). */
intr_disable();

/* In thông tin ra màn hình */
printf("%s: exit(%d)\n", thread_name(), thread_current()->st_exit);

/* Tăng giá trị exit của tiến trình con để thông báo cho tiến trình cha. */
thread_current()->thread_child->store_exit = thread_current()->st_exit;

/* Báo hiệu đã kết thúc cho tiến trình cha bằng cách tăng sema. */
sema_up(&thread_current()->thread_child->sema);

/* Đóng tất cả các tệp đã mở */
struct list_elem *e;
...
while (!list_empty(files)) {
  ...
  //file_close(f->file);
  release_lock_f();
  /* Loại bỏ tệp khỏi danh sách */
  list_remove(e);
  /* Giải phóng tài nguyên của tệp */
  free(f);
}

/* Loại bỏ luồng hiện tại khỏi danh sách tất cả các luồng */
list_remove(&thread_current()->allelem);

/* Đặt trạng thái của luồng là THREAD_DYING và chọn lịch trình. */
